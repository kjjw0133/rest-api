...
+ 앞에 친구 추가 기능을 구현헀으면 다음으로 친구 추가 수락, 거절 기능을 만들어야 함, 메인 페이지 사이드 메뉴에 있는 더보기에 수락,거절 기능 구현

2. 로그인을 하고 로그인 상태에서 회원 가입을 누르면 로그아웃이 되고 로그인 버튼이 2로 변하는 현상 발생, 해결 방법 : 로그인하면 회원가입 버트을 실행하지 못하도록 만든다.
