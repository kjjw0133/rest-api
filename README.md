...
1. 
   + 가끔가다 로그아웃을 해도 메인 폼에 있는 Panel이 사라지지 않는 오류가 있음, 그 상태에서 채팅방에 들어가서 채팅을 치니 일단 친 채팅의 유저 이름은 없었음, DB에는 내가 로그아웃 하기 전 로그인한 계정으로 채팅을 쳤다고 나오고, 서버에서는 아무 이름도 나오지 않음, 서버에서는 누가 채팅을 했는지 표시가 안되고 있는데 DB에서는 로그아웃 전에 로그인한 계정으로 나옴,
     + 발생 원인 : 메인창을 새로 고침 할때 로그아웃을 하면 새로고침 결과를 로그아웃 전에 반영하여서 이런 결과가 나오는 것 같음, 데이터 베이스에 userno가 있는 이유는 채팅창에서 CurrentUser.UserNo 이런 방식으로 값을 받아오는데 이 값이 로그인 할 때 정보를 가져옴, 근데 로그아웃을 해도 이 정보를 초기화하지 않아서 로그인한 정보를 가져와서 로그인이 안되있어도 데이터베이스에서는 어떤 user가 채팅을 쳤는지 나오는 것 같음,
     + 해결 방법
        1. 메인에서 Panel을 클릭할 떄 로그인 정보를 확인하지 않고 바로 입장하는데 이런 오류 상활을 위해서 로그인 확인 로직을 구현
